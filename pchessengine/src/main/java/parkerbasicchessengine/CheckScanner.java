...
public class CheckScanner {
    ...
    private boolean hitByBishop(int col, int row, King king, int kingCol, int kingRow, int colVal, int rowVal) {
    ...
    private boolean hitByKnight(int col, int row, King king, int kingCol, int kingRow) {
    ...
    private boolean checkKnight(Piece piece, King king, int col, int row) {
        return piece != null && !board.sameTeam(piece, king) && piece.name.equals("Knight") && (piece.col != col || piece.row != row);
    }

    private boolean hitByKing(King king, int kingRol, int kingRow) {
        return checkKing(board.getPiece(kingRol - 1, kingRow - 1), king)
                || checkKing(board.getPiece(kingRol + 1, kingRow - 1), king)
                || checkKing(board.getPiece(kingRol, kingRow - 1), king)
                || checkKing(board.getPiece(kingRol - 1, kingRow), king)
                || checkKing(board.getPiece(kingRol + 1, kingRow), king)
                || checkKing(board.getPiece(kingRol - 1, kingRow + 1), king)
                || checkKing(board.getPiece(kingRol + 1, kingRow + 1), king)
                || checkKing(board.getPiece(kingRol, kingRow + 1), king);
    ...
    private boolean checkKing(Piece piece, King king) {
        return piece != null && !board.sameTeam(piece, king) && piece.name.equals("King");
    }

    private boolean hitByPawn(int col, int row, King king, int kingCol, int kingRow) {
    ...
    private boolean checkPawn(Piece piece, King king, int col, int row) {
        return piece != null && !board.sameTeam(piece, king) && piece.name.equals("Pawn") && (piece.col != col || piece.row != row);
    }

    public boolean isGameOver(Piece king) {
        for (Piece piece : board.pieceList) {
            if (board.sameTeam(piece, king)) {
                board.selectedPiece = piece instanceof King ? king : null;
